...
export default function EditEducationPage() {
  ...
  const [formData, setFormData] = useState<
    Omit<Education, "createdAt" | "updatedAt">
  >({
    id: "",
  ...
  return (
    <main className="p-6">
      <form onSubmit={handleSubmit}>
        ...
        {/* 졸업일 */}
        {formData.status !== GraduationStatus.ENROLLED &&
          formData.status !== GraduationStatus.LEAVE && (
            <div className="mb-6">
              <label className="block text-sm font-medium text-dark mb-2">
                졸업일{" "}
                {formData.status === GraduationStatus.EXPECTED
                  ? "(예정일)"
                  : ""}
