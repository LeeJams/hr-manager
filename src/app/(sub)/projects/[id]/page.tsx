  ...
  useEffect(() => {
    ...
    const mockProjects: Record<string, Project> = {
      "1": {
        ...
        company: "테크솔루션",
        overview:
          "AI 플랫폼의 모니터링, 로그처리, 통계용 Data API 개발 프로젝트입니다.",
        ...
        interviewSchedule:
          "프로젝트 지원 > 실무진 인터뷰 > 계약 결정 (인터뷰 진행 후 2~3일 이내 계약 여부 결정)",
        additionalNotes:
          ...
          <div className="grid grid-cols-3 gap-3 pt-4 border-t border-gray-100">
            <div>
              ...
              <p className="text-sm font-medium text-dark">
                {getContractTypeText(project.contractType)}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-1">근무 형태</p>
              <p className="text-sm font-medium text-dark">
                {getWorkTypeText(project.workType)}
              </p>
            </div>
            <div>
        ...
        <div className="bg-white p-6 mb-2 rounded-lg shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {project.requiredSkills.map((skill, idx) => (
              <span
                key={idx}
                className="px-3 py-1.5 bg-blue-50 text-blue-700 text-sm font-medium rounded"
              >
                {skill}
              </span>
            ))}
