...
export default function ScheduleEventDetailPage() {
  ...
  if (loading) {
    return (
      <MainLayout showBackButton backHref="/schedule" headerTitle="일정 상세">
    ...
    <MainLayout showBackButton backHref="/schedule" headerTitle="일정 상세">
      {/* 이벤트 정보 */}
        <div className="bg-white border-b border-gray-200 p-6">
          ...
          <h2 className="text-2xl font-bold text-dark mb-2">{event.projectName}</h2>
          <p className="text-sm text-gray-600 mb-4">{formatDate(event.date)}</p>
          {event.description && <p className="text-sm text-gray-700 leading-relaxed">{event.description}</p>}
        </div>
        ...
        <div className="p-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mb-4">
            <h4 className="text-base font-semibold text-dark mb-3">{project.title}</h4>
            ...
            <div className="space-y-3 pt-3 border-t border-gray-100">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-dark">
                  {project.contractStartDate} ~ {project.contractEndDate || '진행중'}
                </span>
              </div>
